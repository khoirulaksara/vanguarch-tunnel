...
import { Search, Globe, ChevronRight, Activity, XCircle, ArrowDown, ArrowUp } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto">
          {logs.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-[#52525b] p-6 text-center space-y-3">
              ...
              <p className="text-xs">No requests intercepted yet.</p>
              <p className="text-[10px] text-[#3f3f46]">Make sure you enabled Web Inspector when starting a tunnel.</p>
            </div>
          ) : (
            <div className="flex flex-col divide-y divide-[#27272a]">
              {logs.map((log) => (
                <button
                  ...
                  className={cn(
                    "flex flex-col p-3 hover:bg-[#18181b] transition-colors text-left",
                    selectedLogId === log.log_id ? "bg-[#18181b] border-l-2 border-orange-500" : "border-l-2 border-transparent"
                  )}
                >
                  <div className="flex items-center justify-between w-full mb-1">
                    <div className="flex items-center gap-2 font-mono text-xs">
                      <span className={cn("font-bold", getMethodColor(log.request?.method || ''))}>
                        {log.request?.method}
                      </span>
