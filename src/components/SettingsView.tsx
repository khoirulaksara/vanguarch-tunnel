...
export function SettingsView() {
  ...
  const {
    ...
    setEnabled: setHealthEnabled,
    setIntervalMinutes: setHealthInterval,
  ...
  const [promptType, setPromptType] = useState<"workspace" | "single">(
    "workspace",
  );
  ...
  const checkUpdate = async (currentVersionStr: string) => {
    ...
    try {
      ...
      if (match) {
        ...
        const latestNormalized = latest.startsWith("v")
          ? latest.substring(1)
          : latest;
  ...
  const removeDirectory = (
    ...
    if (type === "workspace") {
      setWorkspaceDirectories(
        workspaceDirectories.filter((path) => path !== pathToRemove),
      );
    } else {
      setSingleProjectDirectories(
        singleProjectDirectories.filter((path) => path !== pathToRemove),
      );
    }
  };
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#09090b] text-[#e4e4e7] overflow-y-auto relative">
        ...
        <div className="sticky top-0 z-10 bg-[#09090b] h-16 px-6 border-b border-[#27272a] flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-[#a1a1aa] text-[10px]">
              Configure global preferences and initial setups.
            </p>
          </div>
        </div>
              ...
              <div className="pb-6 border-b border-[#27272a]">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2 text-[10px]">
                    {isChecking ? (
                      <span className="text-[#a1a1aa] uppercase tracking-wider font-bold">
                        Checking...
                      </span>
                    ) : toolVersion ? (
                ...
                {toolVersion && (
                  <div className="mb-4 p-3 bg-[#18181b] border border-[#27272a] rounded flex items-center justify-between">
                    <div>
                      <div className="text-[10px] uppercase text-[#52525b] font-bold mb-1">
                        Version
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="text-xs font-mono text-[#e4e4e7]">
                          {toolVersion.split(" (")[0]}
                        </div>
                        {isCheckingUpdate ? (
                          <span className="text-[9px] text-[#a1a1aa] uppercase tracking-wider font-bold animate-pulse">
                            Checking update...
                          </span>
                        ) : latestVersion === "latest" ? (
                          <span className="text-[9px] bg-green-500/10 text-green-500 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">
                            Latest Version
                          </span>
                        ) : latestVersion ? (
                          <button
                    ...
                    <div>
                      <div className="text-[10px] uppercase text-[#52525b] font-bold mb-1 text-right">
                        Login Status
                      </div>
                      <div className="flex items-center gap-1.5 justify-end">
                        {isLoggedIn ? (
                          ...
                          <span className="text-xs font-bold text-orange-500 uppercase flex items-center gap-1">
                            <XCircle className="w-3.5 h-3.5" /> Not
                            Authenticated
                          </span>
                        )}
                      </div>
                ...
                <p className="text-[10px] text-[#a1a1aa] mb-4 max-w-lg leading-relaxed">
                  If you haven't authenticated Cloudflared on this machine, you
                  need to login before creating named tunnels. This will open a
                  browser window to authenticate with Cloudflare Zero Trust.
                </p>
                    ...
                    <button
                      onClick={() => {
                        ...
                        invoke?.("get_cloudflared_domain")
                          ...
                          .catch((err) => {
                            toast.error("Fetch Domain Error", {
                ...
                {loginStatus && (
                  <p className="mt-2 text-[10px] text-orange-500 font-mono">
                    {loginStatus}
                  </p>
                )}
              </div>
              <div>
                <label className="block text-[10px] uppercase text-[#52525b] mb-1.5 font-bold">
                  Public Root Domain
                </label>
                <input
                  type="text"
                ...
                <p className="text-[10px] text-[#52525b] mt-2 leading-relaxed">
                  Domain you have access to in Cloudflare (e.g.{" "}
                  <code className="bg-black border border-[#27272a] px-1 py-0.5 rounded text-[#a1a1aa] ml-1">
                    serat.us
                  </code>
                  ). This is auto-detected from your Cloudflared certificate
                  when you login.
                </p>
              </div>
                ...
                <p className="text-[10px] text-[#52525b] mt-2 leading-relaxed">
                  {cloudflaredPath.trim() === "" ? (
                    <span className="text-orange-500 font-bold">
                      Currently using: Auto-managed Default (AppData)
                    </span>
                  ) : (
                    <span className="text-orange-500 font-bold">
                      Currently using: Custom path
                    </span>
                  )}
                  <br />
                  If you want to use a custom installation of cloudflared,
                  specify the absolute path to the executable here.
                </p>
              </div>
              ...
              <div className="pt-4 border-t border-[#27272a]">
                ...
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-3">
                    <label className="block text-[10px] uppercase text-[#52525b] font-bold">
                      Workspace Directories (Multiple Projects)
                    </label>
                    <button
                  ...
                  <p className="text-[10px] text-[#52525b] mt-2 leading-relaxed">
                    Directories that contain multiple web projects (e.g.,
                    Laragon's www folder). The app will scan subdirectories
      ...
      <div className="p-5 bg-[#0c0c0e] border border-[#27272a] rounded-xl">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className={cn(
              ...
              healthEnabled ? "bg-green-500" : "bg-[#3f3f46]",
            )}
          >
            <span
              className={cn(
        ...
        <p className="text-[10px] text-[#52525b] leading-relaxed mb-4">
          Automatically pings active tunnels at the selected interval. Sends an
          OS notification when a tunnel goes down or recovers.
        </p>

        <div
          className={cn(
            "flex flex-col gap-4 transition-opacity",
            !healthEnabled && "opacity-40 pointer-events-none",
          )}
        >
          {/* Interval picker */}
          <div>
            <label className="block text-[10px] uppercase text-[#52525b] mb-2 font-bold flex items-center gap-1.5">
              <Clock className="w-3 h-3" /> Check Interval
            </label>
            <div className="flex gap-2">
              {[1, 2, 5, 10].map((min) => (
                <button
                  key={min}
                  onClick={() => setHealthInterval(min)}
                  className={cn(
                    "px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all border",
                    healthInterval === min
                      ? "bg-orange-500/20 text-orange-400 border-orange-500/40"
                      : "bg-[#18181b] text-[#52525b] border-[#27272a] hover:text-[#a1a1aa] hover:border-[#52525b]",
                  )}
                >
                  {min}m
                </button>
            ...
            <p className="text-[10px] text-[#3f3f46] mt-2">
              Each active tunnel's public URL is pinged every {healthInterval}{" "}
              minute{healthInterval > 1 ? "s" : ""}.
            </p>
          </div>

          {/* Info box */}
          <div className="p-3 bg-[#18181b] border border-[#27272a] rounded-lg flex items-start gap-2">
            <Activity className="w-3.5 h-3.5 text-[#52525b] shrink-0 mt-0.5" />
            <p className="text-[10px] text-[#52525b] leading-relaxed">
              Health badges are shown on the Dashboard next to each active
              tunnel. OS notifications require browser notification permission
